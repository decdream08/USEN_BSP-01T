...
{
	 /*Initialize Slave I2C peripheral*/
	if(HAL_I2C_Init(I2C1, 100000/*400000*/) != HAL_OK) //400kbps - Fast mode
	{
		 /* Initialization Error */
     		I2C_Error_Handler();
	}
	/* Set  Own slave address for I2C device */
	HAL_I2C_Slave_SetAddress0(I2C1, I2C1DEV_S_ADDR, DISABLE);
	HAL_I2C_Slave_SetAddress1(I2C1, I2C1DEV_S_ADDR, DISABLE);
}

void I2C1_Interrupt_Write_Data(uint8_t uDeviceId, uint8_t uAddr, uint8_t *uData, uint8_t uDataSize) //Master - I2C_TRANSFER_INTERRUPT
{
	I2C_M_SETUP_Type transferMCfg;
	uint8_t i;
	uint8_t Master_Buf[BUFFER_SIZE] = {0,}; //BUFFER_SIZE = 10;
#if defined(ESD_ERROR_RECOVERY)
	uint32_t uCount = 0;
#ifdef ESD_ERROR_RECOVERY
	Bool B_Error = FALSE;
#endif
	...
	do {
		...
		/* Call I2C function */
		HAL_I2C_Master_Transmit(I2C1, &transferMCfg, I2C_TRANSFER_INTERRUPT);
...
void I2C1_Interrupt_Read_Data(uint8_t uDeviceId, uint8_t uAddr, uint8_t *uData, uint8_t uDataSize) //Master - I2C_TRANSFER_INTERRUPT
{
	I2C_M_SETUP_Type transferMCfg;
	uint8_t regAddr = 0, i;
	uint8_t Master_Buf[BUFFER_SIZE] = {0,}; //BUFFER_SIZE = 10;
	...
	do{
		...
		HAL_I2C_MasterTransferData(I2C1, &transferMCfg, I2C_TRANSFER_INTERRUPT);
